export default class Fragmen {
    ...
    init(option){
        // option check
        if(option == null || option.hasOwnProperty('target') !== true || option.target == null){return;}
        if(option.target instanceof HTMLElement !== true){return;}
        // init canvas
        this.target = this.eventTarget = option.target;
        if(this.target instanceof HTMLCanvasElement === true){
        ...
        // check event
        if(option.hasOwnProperty('eventTarget') === true && option.eventTarget != null){
            this.eventTarget = option.eventTarget;
        }
        if(option.hasOwnProperty('mouse') === true && option.mouse === true){
            this.eventTarget.addEventListener('mousemove', this.mouseMove, false);
        }
        if(option.hasOwnProperty('escape') === true && option.escape === true){
            this.escape = true;
            window.addEventListener('keydown', this.keyDown, false);
        }
        if(option.hasOwnProperty('resize') === true && option.resize === true){
            this.resize = true;
...
void main(){
    ...
    reset(){
        ...
        if(
            this.createShader(this.program, 0, this.VS) !== true ||
            this.createShader(this.program, 1, this.FS) !== true
        ){return;}
    ...
    createShader(p, i, j){
        ...
        if(!this.gl.getShaderParameter(k, this.gl.COMPILE_STATUS)){
            const msg = this.gl.getShaderInfoLog(k);
            console.warn(msg);
            alert(msg);
            return false;
        }
    ...
    /**
     * mouse event
     * @param {MouseEvent} evt - MouseEvent object
     */
    mouseMove(evt){
        let bound, x, y, w, h;
        if(this.eventTarget === window){
            ...
            x = evt.clientX - (bound.left - window.scrollX);
            y = evt.clientY - (bound.top - window.scrollY);
    ...
    /**
     * key evtnt
     * @param {KeyboardEvent} evt - KeyboardEvent object
     */
    keyDown(evt){
        if(this.gl == null){return;}
        this.run = evt.key !== 'Escape';
    }
}
